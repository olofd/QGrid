angular.module('qGrid').controller('qGridController', ['$scope', '$http', '$q', '$timeout', function ($scope: qGrid.IQGridScope<any>, $http: ng.IHttpService, $q: ng.IQService, $timeout) {
    ...
    var gridModel = <qGrid.IGridModel<any>>{
        headerRowHeight: 70,
        footerRowHeight: 40,
    ...
    var qGridSettings = <qGrid.IQGridSettings<any>>{
        buildServerRequestObject: function (search: boolean) {

            var requestIsSearch = false;
            var searchObject = <qGrid.IServerRequestObject>{
                requestInfo: <qGrid.IRequestInfo>{
                    id: this.gridId,
                ...
                requestSearchInfo: <qGrid.IColumnsSearchInfo[]>(function () {
                    var isSearch = false;
                    var soObject = [];
        ...
        requestData: function (searchObject: qGrid.IServerRequestObject) {
        ...
        loadGrid: function () {
            ...
            requestPromise.success((response: qGrid.IServerResponseObject<any>) => {
                var qGrid = <qGrid.IQGridSettings<any>>this;
        ...
        onSort: function (newVal: qGrid.ISortInfo, oldVal: qGrid.ISortInfo) {
            return this.loadGrid();
        },
        onPaging: function (newVal: qGrid.IPagingOptions, oldVal: qGrid.IPagingOptions) {
        ...
        autoComplete: function (val) {
            return $timeout(() => {
                ...
                return $http.post(this.domain + this.urlAutoComplete, searchObject).then(function (res) {
                    return res.data;
                });
            }, 0);
        },
        exportGrid: function (type) {
            ...
            (<any>$).fileDownload(this.domain + this.urlExport, {
                httpMethod: 'POST',
                data: searchObject
            });
    ...
    angular.forEach($scope.qGrid.columnDefs, function (col: qGrid.IColumn) {
        if (!col.headerClass) {
    ...
    (<any>$scope.qGrid.qGridSettings).modelComplete = true;
    $scope.qGrid.qGridSettings.loadGrid();

    $scope.$watch('qGrid.sortInfo', function (newVal: qGrid.ISortInfo, oldVal: qGrid.ISortInfo) {
        if (newVal.directions[0] !== oldVal.directions[0] || newVal.fields[0] !== oldVal.fields[0]) {
            $scope.qGrid.qGridSettings.onSort(newVal, oldVal);
        }
    }, true);

    $scope.$watch('qGrid.pagingOptions', function (newVal: qGrid.IPagingOptions, oldVal: qGrid.IPagingOptions) {
        console.log("RunPag");
        if (newVal !== oldVal) {
